...
| **08** | Autenticación (JWT + SecureStore + Zustand) | [`week-08`](../../tree/week-08) | ✅ Entregado |
| **09** | Animaciones Básicas (Animated + LayoutAnimation) | [`week-09`](../../tree/week-09) | ✅ Entregado |
...
**Objetivo:** inicio de sesión completo con JWT — login y registro con
React Hook Form + Zod, tokens (access y refresh) en **SecureStore**
(nunca en AsyncStorage), estado de sesión con **Zustand + persist**,
navegación condicional Auth ↔ App y **renovación automática del token**
con un interceptor de Axios.

**Entidad usada:** `students` (el usuario que inicia sesión) con sus
`courses` y `enrollments` (cursos inscritos, simulados), sobre la API de
práctica dummyjson.com.

**Ruta del proyecto en esta rama:**
```
week-08-autenticacion/3-proyecto/starter/
```

**Cómo ejecutarlo:** ver el `README.md` dentro de esa carpeta — incluye
la guía de estudio (JWT, access vs refresh token, interceptor 401,
hidratación de la sesión) y qué se probó y qué no.

**Stack:** React Native · Expo SDK 57 · Zustand · Axios · SecureStore · React Hook Form · Zod · TypeScript

---

## 🎓 Semana 09 — Animaciones Básicas

**Objetivo:** "Mis cursos" con las 5 animaciones que pide la semana,
todas con la API `Animated` y `LayoutAnimation` de React Native (sin
librerías extra): entrada del detalle (`parallel`), rebote al tocar
(`spring`), barra de progreso con ancho y color (`interpolate`), cascada
de tarjetas (`stagger`) y cambios de lista animados (`LayoutAnimation`).

**Entidad usada:** `enrollments` (mis cursos inscritos) con `lessons`
(el progreso es lecciones completadas / total) y `courses`.

**Ruta del proyecto en esta rama:**
```
week-09-animaciones_basicas/3-proyecto/starter/
```

**Cómo ejecutarlo:** ver el `README.md` dentro de esa carpeta — incluye
la guía de estudio (`timing` vs `spring`, `interpolate`, por qué la
barra usa `useNativeDriver: false`, un bug real de la cascada) y qué se
midió y qué no.

**Stack:** React Native · Expo SDK 57 · Animated · LayoutAnimation · TypeScript
